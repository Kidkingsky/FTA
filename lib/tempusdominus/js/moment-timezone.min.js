!(function (M, z) {
  ...
  "object" == typeof module && module.exports
    ? (module.exports = z(require("moment")))
  ...
  function o(M) {
    var z = 0,
      ...
      c = 0,
      b = 1;
    for (45 === M.charCodeAt(0) && (b = -(z = 1)); z < p.length; z++)
      c = 60 * c + q(p.charCodeAt(z));
    for (z = 0; z < O.length; z++) (A /= 60), (c += q(O.charCodeAt(z)) * A);
    return c * b;
  }
  function a(M) {
    for (var z = 0; z < M.length; z++) M[z] = o(M[z]);
  }
  function L(M, z) {
    for (var b = [], p = 0; p < z.length; p++) b[p] = M[z[p]];
    return b;
  }
  function n(M) {
    for (
      ...
      O[c] = Math.round((O[c - 1] || 0) + 6e4 * O[c]);
    return (
      (O[A - 1] = 1 / 0),
      {
        name: M[0],
  ...
  function i() {
    for (
      ...
      (b = new Date(p, q, 1).getTimezoneOffset()) !== A &&
        ((M = (function (M, z) {
          for (var b; (b = 6e4 * (((z.at - M.at) / 12e4) | 0)); )
            (b = new N(new Date(M.at + b))).offset === M.offset
              ? (M = b)
              : (z = b);
          return M;
        })(O, (z = new N(new Date(p, q, 1))))),
        c.push(M),
        c.push(new N(new Date(M.at + 6e4))),
        (O = z),
        (A = b));
    for (q = 0; q < 4; q++)
      c.push(new N(new Date(p + q, 0, 1))),
        c.push(new N(new Date(p + q, 6, 1)));
    return c;
  }
  function u(M, z) {
  ...
  function e() {
    ...
    for (
      var b,
        ...
        c = (function (M) {
          for (
            var z, b, p, O = M.length, A = {}, c = [], q = {}, o = 0;
            o < O;
            o++
          )
            if (((b = M[o].offset), !q.hasOwnProperty(b))) {
              for (z in (p = R[b] || {})) p.hasOwnProperty(z) && (A[z] = !0);
              q[b] = !0;
            }
          for (o in A) A.hasOwnProperty(o) && c.push(d[o]);
          return c;
        })(O),
        q = [],
        o = 0;
      ...
      for (b = new X(t(c[o])), p = 0; p < A; p++) b.scoreOffsetAt(O[p]);
      q.push(b);
    }
    return q.sort(u), 0 < q.length ? q[0].zone.name : void 0;
  }
  function r(M) {
  ...
  function T(M) {
    ...
    for ("string" == typeof M && (M = [M]), z = 0; z < M.length; z++) {
      ...
      var A,
        ...
        q = O,
        o = p[2].split(" ");
      for (a(o), A = 0; A < o.length; A++)
        (c = o[A]), (R[c] = R[c] || {}), (R[c][q] = !0);
    }
  }
  function t(M, z) {
    M = r(M);
    ...
    return b instanceof f
      ...
      ? ((b = new f(b)), (W[M] = b))
      : A[M] && z !== t && (z = t(A[M], t))
      ? ((b = W[M] = new f())._set(z), (b.name = d[M]), b)
      : null;
  }
  function l(M) {
    var z, b, p, O;
  ...
  function S(M) {
    var z = Array.prototype.slice.call(arguments, 0, -1),
      ...
      z = O.utc.apply(null, z);
    return (
      !O.isMoment(M) && E(z) && (M = t(b)) && z.add(M.parse(z), "minutes"),
      z.tz(b),
      z
    );
  }
  (b < 2 || (2 == b && p < 6)) &&
    ...
    (f.prototype = {
      ...
      _index: function (M) {
        M = (function (M, z) {
          var b,
            p = z.length;
          if (M < z[0]) return 0;
          if (1 < p && z[p - 1] === 1 / 0 && M >= z[p - 2]) return p - 1;
          if (M >= z[p - 1]) return -1;
          for (var O = 0, A = p - 1; 1 < A - O; )
            z[(b = Math.floor((O + A) / 2))] <= M ? (O = b) : (A = b);
          return A;
        })(+M, this.untils);
        if (0 <= M) return M;
      },
      countries: function () {
        var z = this.name;
        return Object.keys(c).filter(function (M) {
          return -1 !== c[M].zones.indexOf(z);
        });
      },
      parse: function (M) {
        for (
    ...
    (S.names = function () {
      var M,
        z = [];
      for (M in d)
        d.hasOwnProperty(M) && (W[M] || W[A[M]]) && d[M] && z.push(d[M]);
      return z.sort();
    }),
    (S.Zone = f),
    (S.unpack = n),
    ...
    (S.zonesForCountry = function (M, z) {
      var b;
      return (
        (b = (b = M).toUpperCase()),
        (M = c[b] || null)
  ...
  (O.tz = S),
    ...
    (O.updateOffset = function (M, z) {
      ...
      void 0 === M._z &&
        (p &&
          ...
          !M._isUTC &&
          M.isValid() &&
          ((M._d = O.utc(M._a)._d), M.utc().add(p.parse(M), "minutes")),
        (M._z = p)),
        M._z &&
          ((p = M._z.utcOffset(M)),
          Math.abs(p) < 16 && (p /= 60),
          void 0 !== M.utcOffset
            ? ((b = M._z), M.utcOffset(-p, z), (M._z = b))
            : M.zone(p, z));
    }),
    (M.tz = function (M, z) {
      if (M) {
        if ("string" != typeof M)
          throw new Error(
            "Time zone name must be a string, got " + M + " [" + typeof M + "]"
          );
        return (
          (this._z = t(M)),
          this._z
            ? O.updateOffset(this, z)
            : C(
    ...
    (M.zoneName = P(M.zoneName)),
    (M.zoneAbbr = P(M.zoneAbbr)),
    (M.utc = D(M.utc)),
    (M.local = D(M.local)),
    (M.utcOffset =
      ((g = M.utcOffset),
      function () {
        return (
          0 < arguments.length && (this._z = null), g.apply(this, arguments)
        );
      })),
    (O.tz.setDefault = function (M) {
      return (
        ...
        (O.defaultZone = M ? t(M) : null),
        O
      );
    });
  M = O.momentProperties;
  return (
